...
Für planare, konjugierte (d.h. aus sp:sup:`2`-hybridisierten Atomen aufgebaute) Systeme können folgende Näherungen eingeführt werden:
...
In der Praxis wird :math:`\alpha` für Kohlenstoff willkürlich auf Null gesetzt, und die Parameter anderer Atome werden relativ dazu empirisch bestimmt. :math:`\beta` (Resonanzintegral) wird für eine C–C Bindung der Länge 1.4 \ :math:`\text{Å}` als Standard auf -1 gesetzt. Bei Molekülen mit starken Abweichungen von dieser Standardlänge oder bei C–X Bindungen wird der :math:`\beta`-Wert empirisch verändert.
